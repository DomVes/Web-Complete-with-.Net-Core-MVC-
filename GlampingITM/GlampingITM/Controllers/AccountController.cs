    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    ModelState.AddModelError(string.Empty, "Ha superado el máximo número de intentos, intente de nuevo en 2 minutos.");
                }
                else if (result.IsNotAllowed)
                {
                    ModelState.AddModelError(string.Empty, "Usuario inhabilitado, sigue las instrucciones enviadas a tu correo electrónico para habilitar su cuenta.");
                }
                else
                {
        ...
        public async Task<IActionResult> RecoverPassword(RecoverPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                _mailHelper.SendMail(
                    $"{user.FullName}",
                    model.Email,
                    "Glamping ITM - Recuperación de Contraseña",
                    $"<h1>GLAMPINGITM - Recuperación de Contraseña</h1>" +
