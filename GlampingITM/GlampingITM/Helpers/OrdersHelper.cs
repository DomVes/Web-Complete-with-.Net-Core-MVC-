    ...
    public class OrdersHelper : IOrdersHelper
    {
        ...
        private async Task<Response> CheckInventoryAsync(ShowCartViewModel model)
        {
            ...
            foreach (TemporalSale? item in model.TemporalSales)
            {
                ...
                {
                    response.IsSuccess = false;
                    response.Message = $"Lo sentimos no tenemos existencias suficientes del Glamping {item.Product.Name}, para tomar su pedido. Por favor disminuir la cantidad o sustituirlo por otro.";
                    return response;
                }
            }
            return response;
